...
```r
for (s in c(3, 4, 5)) {
    gut <- pnorm(12, mean = mu, sd = s, lower.tail = FALSE)
    output.gut <- paste("Bei einer Standabweichung von", s, "haben", gut * 100, 
        "% eine Note im Einer-Bereich.")
    print(output.gut)
}
```

```
## [1] "Bei einer Standabweichung von 3 haben 9.12112197258678 % eine Note im Einer-Bereich."
## [1] "Bei einer Standabweichung von 4 haben 15.8655253931457 % eine Note im Einer-Bereich."
## [1] "Bei einer Standabweichung von 5 haben 21.1855398583397 % eine Note im Einer-Bereich."
```


Wie steht die Anzahl guter Noten in Beziehung zur Anzahl schlechter Noten? 

Es gibt eine deutlich höhere Durchfallquote im Verhältnis zu der Anzahl der Noten im Einer-Bereich.

Warum?

Es gibt doppelt so viele Möglichkeiten durchzufallen - in totalen Zahlen: 3 Noten für den Einer-Bereich und 6 Noten für den Durchgefallen-Bereich. Bei einer Vergrößerung der Standardabwichung schlägt sich dieses Verhältnis auch in den Prozentzahlen bezüglich Durchfallquote und Einer-Bereich-Quote nieder.  
...
Die Notenverteilung ist dann linksschief.
...
2. Wie viele Studenten bekommen zumindest 10 NP?
    ...
    18.4721  Studenten bekommen zumindest 10 Notenpunkte.

3. Wie viele Studenten bekommen weniger als 10 NP?

    31.5279 Studenten bekommen weniger als 10 Notenpunkte.

4. Wie viele Studenten bekommen weniger als 8 NP?

    18.4721 Studenten bekommen weniger als 8 Notenpunkte.
...
Es sollte ein ungerichteter Test (2-seitig) genutzt werden, da erst noch herausgefunden werden muss, ob überhaupt ein Unterschied zu anderen Dozenten bezüglich der Notengbung besteht. Würde man bereits wissen, ob der Dozent eher bessere oder eher schlechtere Noten zu geben scheint, wäre ein gerichteter Test sinnvoll.

In einem kleinen Seminar mit 7 Studenten beträgt der Durchschnittswert 10. Unterscheidet sich der Notenspiegel von dem idealen? Berechnen Sie den $z$-Test: 1.7638 $<$ 1.8808. 
...
Die Benotung durch den Dozenten scheint bei 50 Teilnehmern nicht der idealisierten Notenverteilung zu entsprechen. Bei 7 Teilnehmer jedoch, unterscheidet sich die Notenverteilung dieses Dozenten nicht signifikant von der idealisierten Notenverteilung.
...
Da bereits bekannt ist, dass der Dozent eher schlechtere Noten gäbe, kann hier ein gerichteter, also einseitiger Test benutzt werden.
