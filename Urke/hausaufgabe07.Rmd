...
for(s in c(3,4,5) ){
  ...
  output.gut <- paste("Bei einer Standabweichung von",s, "haben",gut*100,"% eine Note im Einer-Bereich.")
  print(output.gut)
}
```
...
3. Wie viele Studenten bekommen weniger als 10 NP?
    ...
    `r (pnorm(9,mean=mu,sd=3, lower.tail=TRUE)) * n` Studenten bekommen weniger als 10 Notenpunkte.

4. Wie viele Studenten bekommen weniger als 8 NP?

    `r (pnorm(7,mean=mu,sd=3, lower.tail=TRUE)) * n` Studenten bekommen weniger als 8 Notenpunkte.
...
In einem kleinen Seminar mit 7 Studenten beträgt der Durchschnittswert 10. Unterscheidet sich der Notenspiegel von dem idealen? Berechnen Sie den $z$-Test: `r sqrt(7)*((10-8)/3)` $<$ `r qnorm(0.97)`. 
...
Berechnen Sie den $z$-Test:
    ...
    `r sqrt(20)*((7-8)/3)`
    `r abs(sqrt(20)*((7-8)/3))` <= `r qnorm(0.94)`
...
Berechnen Sie den $z$-Test:
    ...
    `r sqrt(25)*((7-8)/3)`
    `r abs(sqrt(25)*((7-8)/3))` > `r qnorm(0.94)`
